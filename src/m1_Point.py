...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance = self.distance + ((x - self.x) ** 2 + (y - self.y) ** 2) ** 0.5
        self.x = x
        self.y = y
        self.n = self.n + 1

    def move_by(self, x, y):
        self.distance = self.distance + (x ** 2 + y ** 2) ** 0.5
        self.x = self.x + x
        self.y = self.y + y
        self.n = self.n + 1
    ...
    def get_distance_from_start(self):
        return ((self.start_x - self.x) ** 2 + (self.start_y - self.y) ** 2) ** 0.5

    def get_distance_traveled(self):
        return self.distance

    def closer_to(self, a, b):
        d2 = ((a.x - self.x) ** 2 + (a.y - self.y) ** 2) ** 0.5
        d3 = ((b.x - self.x) ** 2 + (b.y - self.y) ** 2) ** 0.5
        if d2 < d3:
            return a
        else:
            return b
